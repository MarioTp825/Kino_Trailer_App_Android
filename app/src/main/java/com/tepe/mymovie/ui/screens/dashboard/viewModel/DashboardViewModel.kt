package com.tepe.mymovie.ui.screens.dashboard.viewModel
...
class DashboardViewModel @Inject constructor(
    ...
    private fun getSchema() = viewModelScope.launch {
        ...
        if (data == null) {
            ...
            _dashboardState.value = NetworkState.Error(message)
            return@launch
        }
    ...
    fun isMovieFavorite(movie: MovieUI, launchMovie: (MovieUI) -> Unit) =
        viewModelScope.launch(Dispatchers.IO) {
            val movieFound = movieLocalRepository.getMovieById(movie.id)
            withContext(Dispatchers.Main) {
                launchMovie(
                    movie.copy(favorite = movieFound != null)
                )
            }
        }
}